...
export default function TargetBeneficiaries() {
  ...
  async function handleAdd(e) {
    ...
    try {
      const row = await api.addTargetBeneficiary(newName.trim());
  ...
  async function handleToggle(item) {
    ...
    try {
      await api.setTargetBeneficiaryStatus(item.id, next);
  ...
  async function handleRename(item) {
    ...
    setBusy(true);
    try {
      const updated = await api.renameTargetBeneficiary(item.id, editName.trim());
  ...
  async function handleDelete(item) {
    setBusy(true);
    try {
      await api.deleteTargetBeneficiary(item.id);
  ...
  return (
    <div className="page" style={{ maxWidth: 780, paddingBottom: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4, gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: 19, fontWeight: 900, color: '#0B1E3D', margin: 0 }}>Target Beneficiaries</h1>
          <p style={{ margin: '3px 0 0', fontSize: 12, color: '#9CA3AF' }}>
            Beneficiary categories used to tag training programmes, schemes and placement drives.
          </p>
        </div>
      ...
      {/* ── Info banner ── */}
      <div style={{ background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: 8, padding: '8px 14px', marginBottom: 16, marginTop: 14, display: 'flex', gap: 8, alignItems: 'center' }}>
        <span style={{ fontSize: 14 }}>🎯</span>
        <span style={{ fontSize: 12, color: '#C2410C', fontWeight: 600 }}>
          These categories are used to filter schemes, job postings and courses by target group, and appear in impact reports for government stakeholders.
        </span>
      </div>
      ...
      <div className="card shadow" style={{ padding: '12px 16px', marginBottom: 14 }}>
        <form onSubmit={handleAdd} style={{ display: 'flex', gap: 8, marginBottom: addErr ? 6 : 10 }}>
          <input
            ...
            placeholder="Add new beneficiary group (e.g. Transgender Persons)…"
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', color: '#9CA3AF', padding: 40, fontSize: 13 }}>
          {items.length === 0 ? 'No beneficiary groups yet.' : 'Nothing matches your filter.'}
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          {filtered.map(item => (
            <div key={item.id} style={{
              ...
              boxShadow: item.is_enabled ? '0 1px 4px rgba(0,0,0,.05)' : 'none',
              ...
              <div onClick={() => handleToggle(item)}
                ...
                <div style={{ position: 'absolute', top: 2, left: item.is_enabled ? 14 : 2,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {editId === item.id ? (
                  <input autoFocus value={editName}
                    ...
                    style={{ width: '100%', padding: '2px 6px', borderRadius: 5, border: '1.5px solid #93C5FD', fontSize: 12, fontWeight: 600 }}
                  />
                ) : (
                ...
                <div style={{ display: 'flex', gap: 3, flexShrink: 0 }}>
                  {editId !== item.id && (
                    <button onClick={() => { setEditId(item.id); setEditName(item.name); }}
                      title="Rename"
                      style={{ width: 22, height: 22, borderRadius: 5, border: '1px solid #E5E7EB', background: '#F8FAFC', cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      ✏️
                    </button>
                  )}
                  <button onClick={() => setConfirmDel(item)}
                    title="Delete"
                    style={{ width: 22, height: 22, borderRadius: 5, border: '1px solid #FECACA', background: '#FEF2F2', cursor: 'pointer', fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    🗑
                  </button>
                </div>
              )}
      ...
      {filtered.length > 0 && (
        <div style={{ marginTop: 10, fontSize: 11, color: '#9CA3AF', textAlign: 'right' }}>
          {filtered.length} of {items.length} beneficiary groups
        </div>
      )}
      ...
      {confirmDel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card shadow" style={{ width: 360, maxWidth: '92vw', padding: 24, textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 15, color: '#0B1E3D', marginBottom: 6 }}>Delete Beneficiary Group?</div>
