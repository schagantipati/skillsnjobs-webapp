...
export default function OrgClassifications() {
  ...
  async function handleToggle(item) {
    ...
    try {
      await api.setOrgClassificationStatus(item.id, next);
  ...
  return (
    <div className="page" style={{ maxWidth: 780, paddingBottom: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: 19, fontWeight: 900, color: '#0B1E3D', margin: 0 }}>Organisation Classifications</h1>
          <p style={{ margin: '3px 0 0', fontSize: 12, color: '#9CA3AF' }}>Organisation types used platform-wide. Enable or disable values as needed.</p>
        </div>
        {/* Stats chips */}
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          {[
            ...
            <div key={c.label} style={{ background: c.bg, borderRadius: 8, padding: '6px 14px', textAlign: 'center', minWidth: 60 }}>
      ...
      <div className="card shadow" style={{ padding: '12px 16px', marginBottom: 14 }}>
        <form onSubmit={handleAdd} style={{ display: 'flex', gap: 8, marginBottom: addErr ? 6 : 10 }}>
          <input
            ...
            placeholder="Add new classification (e.g. Skill Development Institute)…"
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', color: '#9CA3AF', padding: 40, fontSize: 13 }}>
          {items.length === 0 ? 'No classifications yet.' : 'Nothing matches your filter.'}
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {filtered.map(item => (
            <div key={item.id} style={{
              ...
              transition: 'all .15s',
              opacity: item.is_enabled ? 1 : 0.7,
            }}>

              {/* Toggle switch */}
              <div onClick={() => handleToggle(item)} title={item.is_enabled ? 'Click to disable' : 'Click to enable'}
                style={{ width: 36, height: 20, borderRadius: 10, cursor: 'pointer', flexShrink: 0, transition: 'background .2s', position: 'relative',
                  background: item.is_enabled ? '#059669' : '#D1D5DB' }}>
                <div style={{ position: 'absolute', top: 2, left: item.is_enabled ? 18 : 2, width: 16, height: 16, borderRadius: '50%', background: '#fff', transition: 'left .2s', boxShadow: '0 1px 3px rgba(0,0,0,.2)' }} />
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {editId === item.id ? (
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: item.is_enabled ? '#0B1E3D' : '#9CA3AF',
                    textDecoration: item.is_enabled ? 'none' : 'line-through', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {item.name}
                  </div>
                )}
              </div>
              ...
              <span style={{ fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 4, flexShrink: 0,
                background: item.is_system ? '#EDE9FE' : '#F1F5F9',
                color:      item.is_system ? '#5B21B6'  : '#94A3B8' }}>
              ...
              <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                {!item.is_system && editId !== item.id && (
                  <button onClick={() => { setEditId(item.id); setEditName(item.name); }}
                    title="Rename"
                    style={{ width: 26, height: 26, borderRadius: 6, border: '1px solid #E5E7EB', background: '#F8FAFC', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1E5FBF' }}>
                    ✏️
                  </button>
                )}
                {!item.is_system && (
                  <button onClick={() => setConfirmDel(item)}
                    title="Delete"
                    style={{ width: 26, height: 26, borderRadius: 6, border: '1px solid #FECACA', background: '#FEF2F2', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#B91C1C' }}>
                    🗑
                  </button>
                )}
              </div>
      ...
      {filtered.length > 0 && (
        <div style={{ marginTop: 10, fontSize: 11, color: '#9CA3AF', textAlign: 'right' }}>
          {filtered.length} of {items.length} classifications
        </div>
      )}
      ...
      {confirmDel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card shadow" style={{ width: 360, maxWidth: '92vw', padding: 24, textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 15, color: '#0B1E3D', marginBottom: 6 }}>Delete Classification?</div>
