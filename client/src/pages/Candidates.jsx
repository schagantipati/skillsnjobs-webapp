...
const STATUS_STYLE = {
  ...
  'in_training': { bg: '#E8F0FB', color: '#003366', label: 'In Training' },
...
const DOMAIN_COLORS = {
  'IT': '#003366', 'it': '#003366', 'Healthcare': '#007B5E', 'Construction': '#D97706',
...
export default function Candidates() {
  ...
  const PILL = (label) => ({
    ...
    background: filter === label ? '#003366' : '#E2E8F0',
    color: filter === label ? '#fff' : '#334155',
  });
  ...
  return (
    <div className="page" style={{ maxWidth: 1200, paddingBottom: 40 }}>
      ...
      <div style={{ marginBottom: 22 }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: '#003366', margin: 0 }}>Candidate Portal</h1>
        <p style={{ margin: '4px 0 0', fontSize: 13, color: '#64748B' }}>Complete candidate lifecycle management — Registration to Placement</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 0, marginBottom: 24, border: '1px solid #E5E7EB', borderRadius: 12, overflow: 'hidden' }}>
        {[
          { val: total,    label: 'Total Registered',  sub: `${rows.length} in database`,  color: '#003366', prefix: '',  borderRight: true },
          ...
          <div key={i} style={{ padding: '20px 24px', background: '#fff', borderRight: card.borderRight ? '1px solid #E5E7EB' : 'none', borderTop: `3px solid ${card.color}` }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 13, color: '#003366', marginTop: 4 }}>{card.label}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontWeight: 800, fontSize: 15, color: '#003366' }}>Candidate Registry</span>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search by name, email, state…"
            style={{ padding: '8px 14px', borderRadius: 8, border: '1.5px solid #E2E8F0', fontSize: 13, width: 240, background: '#F8FAFC' }} />
      ...
      <div className="card shadow" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <thead>
            <tr style={{ background: '#F8FAFC', borderBottom: '1.5px solid #E5E7EB' }}>
              {['CANDIDATE','STATE','DOMAIN','STATUS','AI MATCH','SALARY','ACTION'].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', fontSize: 11, fontWeight: 800, color: '#64748B', letterSpacing: 0.6, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading && (
              <tr><td colSpan={7} style={{ padding: 32, textAlign: 'center', color: '#64748B' }}>Loading candidates…</td></tr>
            )}
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: '#64748B', fontSize: 13 }}>
            ...
            {filtered.map((c, i) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom: '1px solid #E2E8F0', background: i % 2 === 0 ? '#fff' : '#F8FAFC', transition: 'background .1s' }}
                  onMouseEnter={e => e.currentTarget.style.background = '#F0F6FF'}
                  onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? '#fff' : '#F8FAFC'}>

                  {/* Candidate */}
                  <td style={{ padding: '13px 16px' }}>
                    <div style={{ fontWeight: 700, color: '#003366', fontSize: 13 }}>{c.first_name ? `${c.first_name} ${c.last_name || ''}`.trim() : c.name}</div>
                    {qual && <div style={{ fontSize: 11, color: '#64748B', marginTop: 2 }}>{qual}</div>}
                  </td>

                  {/* State */}
                  <td style={{ padding: '13px 16px', color: '#334155', fontWeight: 500 }}>{state}</td>
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 100 }}>
                      <div style={{ flex: 1, height: 5, background: '#E5E7EB', borderRadius: 4, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${score}%`, background: score >= 85 ? '#065F46' : score >= 70 ? '#003366' : '#D97706', borderRadius: 4 }} />
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#003366', minWidth: 30 }}>{score}%</span>
                    </div>
                  </td>

                  {/* Salary */}
                  <td style={{ padding: '13px 16px', color: '#334155', fontWeight: 600 }}>{salary}</td>

                  {/* Action */}
                  <td style={{ padding: '13px 16px' }}>
                    <button onClick={() => setViewCandidate(c)}
                      style={{ background: 'none', border: '1px solid #E2E8F0', borderRadius: 7, padding: '5px 14px', fontSize: 12, fontWeight: 700, color: '#003366', cursor: 'pointer', transition: 'all .15s' }}
                      onMouseEnter={e => { e.currentTarget.style.background = '#003366'; e.currentTarget.style.color = '#fff'; }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = '#003366'; }}>
                      View
                    </button>
                  </td>
                </tr>
        ...
        {filtered.length > 0 && (
          <div style={{ padding: '10px 16px', borderTop: '1px solid #E2E8F0', fontSize: 12, color: '#64748B', background: '#F8FAFC' }}>
      ...
      {viewCandidate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card shadow" style={{ width: 520, maxWidth: '95vw', padding: 28, position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
            <button onClick={() => setViewCandidate(null)} style={{ position: 'absolute', top: 14, right: 16, background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#64748B' }}>×</button>
            <div style={{ display: 'flex', gap: 14, marginBottom: 20, alignItems: 'center' }}>
              <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#003366', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: '#fff', fontWeight: 800, flexShrink: 0 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>{viewCandidate.qualification || ''} · {viewCandidate.state_name || viewCandidate.location || '—'}</div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              {[
                ...
                ['DOB',            formatDate(viewCandidate.dob) || '—'],
                ...
                <div key={k} style={{ background: '#F8FAFC', borderRadius: 8, padding: '10px 12px' }}>
                  ...
                  <div style={{ fontSize: 13, color: '#003366', fontWeight: 600, wordBreak: 'break-all' }}>{v}</div>
                </div>
              ))}
            </div>
            {(Array.isArray(viewCandidate.skills) ? viewCandidate.skills : []).length > 0 && (
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', marginBottom: 8 }}>Skills</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {viewCandidate.skills.map(s => (
                    <span key={s} style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: '#E8F0FB', color: '#003366', border: '1px solid #BFDBFE' }}>{s}</span>
                  ))}
                </div>
              </div>
            )}
            {viewCandidate.bio && (
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', marginBottom: 6 }}>Bio</div>
                <div style={{ fontSize: 13, color: '#334155', lineHeight: 1.6 }}>{viewCandidate.bio}</div>
              </div>
            )}
          </div>
        </div>
...
function AddCandidateModal({ onClose, onSaved }) {
  ...
  const INP = { width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #E2E8F0', fontSize: 13, boxSizing: 'border-box', background: '#F8FAFC' };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="card shadow" style={{ width: 560, maxWidth: '95vw', padding: 28, position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ fontWeight: 800, fontSize: 16, color: '#003366', marginBottom: 20 }}>Add Candidate</div>
        {fields.map((row, ri) => (
        ...
        <div style={{ marginBottom: 12 }}>
          <label style={{ fontSize: 12, fontWeight: 700, color: '#334155', display: 'block', marginBottom: 4 }}>Employment Status</label>
