...
export default function GeographicCoverage() {
  ...
  async function handleAdd(e) {
    ...
    try {
      const row = await api.addGeographicCoverage(newName.trim());
  ...
  async function handleToggle(item) {
    ...
    try {
      await api.setGeographicCoverageStatus(item.id, next);
  ...
  async function handleRename(item) {
    ...
    setBusy(true);
    try {
      const updated = await api.renameGeographicCoverage(item.id, editName.trim());
  ...
  async function handleDelete(item) {
    setBusy(true);
    try {
      await api.deleteGeographicCoverage(item.id);
  ...
  return (
    <div className="page" style={{ maxWidth: 780, paddingBottom: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4, gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: 19, fontWeight: 900, color: '#0B1E3D', margin: 0 }}>Geographic Coverage</h1>
          <p style={{ margin: '3px 0 0', fontSize: 12, color: '#9CA3AF' }}>
            Coverage dimensions used to describe operational reach of training partners and organisations.
          </p>
        </div>
      ...
      {/* ── Info banner ── */}
      <div style={{ background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 8, padding: '8px 14px', marginBottom: 16, marginTop: 14, display: 'flex', gap: 8, alignItems: 'center' }}>
        <span style={{ fontSize: 14 }}>🗺️</span>
        <span style={{ fontSize: 12, color: '#15803D', fontWeight: 600 }}>
          These values appear as coverage tags on training partner profiles and are used for filtering in government dashboards and reports.
        </span>
      </div>
      ...
      <div className="card shadow" style={{ padding: '12px 16px', marginBottom: 14 }}>
        <form onSubmit={handleAdd} style={{ display: 'flex', gap: 8, marginBottom: addErr ? 6 : 10 }}>
          <input
            ...
            placeholder="Add new coverage type (e.g. Hill District Coverage)…"
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', color: '#9CA3AF', padding: 40, fontSize: 13 }}>
          {items.length === 0 ? 'No coverage types yet.' : 'Nothing matches your filter.'}
        </div>
      )}
          ...
          {filtered.map(item => (
            <div key={item.id} style={{
              ...
              <span style={{ fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 4, flexShrink: 0,
                background: item.is_system ? '#D1FAE5' : '#F1F5F9',
                color:      item.is_system ? '#065F46'  : '#94A3B8' }}>
      ...
      {filtered.length > 0 && (
        <div style={{ marginTop: 10, fontSize: 11, color: '#9CA3AF', textAlign: 'right' }}>
          {filtered.length} of {items.length} coverage types
        </div>
      )}
      ...
      {confirmDel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card shadow" style={{ width: 360, maxWidth: '92vw', padding: 24, textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 800, fontSize: 15, color: '#0B1E3D', marginBottom: 6 }}>Delete Coverage Type?</div>
